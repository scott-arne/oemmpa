...
```python
from oemmpa import Analyzer, configure_wizepairz

analyzer = Analyzer(method="wizepairz")
configure_wizepairz(
    analyzer,
    mcs_identity_fraction=0.85,
...
```python
pair = pairs[0]
print(pair.environment_smirks)  # e.g., "[cH:1]([cH:2])[CH3:3]>>[cH:1]([cH:2])[OH:3]"
```

This field is `None` for fragmentation-based pairs, which do not have explicit
...
field is `None` for fragmentation-based rule environments:

```python
from oemmpa import rule_environment_statistics

stats = rule_environment_statistics(analyzer, "pIC50")
row = stats[0]
print(row.explicit_smirks)  # e.g., "[cH:1]([cH:2])[CH3:3]>>[cH:1]([cH:2])[OH:3]"
```
